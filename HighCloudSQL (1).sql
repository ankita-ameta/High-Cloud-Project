...
alter table maindata rename column `Month (#)` to Month_New;
alter table maindata add column date_new date AS 
...
from maindata group by Carrier_Name order by Passengers desc limit 10;
...
from maindata group by `From - To City` order by count(`From - To City`) desc limit 10;
